...
S3Mock.prototype = {
		...
		if (!callback) {
			...
			fs.readFile(search.Bucket + '/' + search.Key, function (err, data) {
				if (!err) {
					...
					if (self.objectExtraDictionary[search.Key]) {
						Object.assign(props, self.objectExtraDictionary[search.Key])
					}
					else {
						props.ContentType = mime.lookup(search.Key) || 'application/octet-stream'
					}
	...
	putObject: function (search, callback) {
		...
		if (search.ContentType || search.ContentDisposition) {
			this.objectExtraDictionary[search.Key] = {
				ContentType: search.ContentType,
				ContentDisposition: search.ContentDisposition
			}
		}
