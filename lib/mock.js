...
S3Mock.prototype = {
		...
		if (!callback) {
			...
			fs.readFile(search.Bucket + '/' + search.Key, function (err, data) {
				if (!err) {
					...
					props.ContentType = self.objectContentTypeDictionary[search.Key] || mime.lookup(search.Key) || 'application/octet-stream'
	...
	putObject: function (search, callback) {
		...
		if (search.ContentType) {
			this.objectContentTypeDictionary[search.Key] = search.ContentType
		}
